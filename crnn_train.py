...
def init_args():
    ...
    args.add_argument('--trainpath', help='training lmdb data path', default='/media/hzl/file/Data-Set/360_data-set/lmdb_360/')
    args.add_argument('--valpath', help='verify lmdb data path', default='/media/hzl/file/Data-Set/360_data-set/lmdb_360_test/')
    ...
    for total_steps in range(params.niter):
        ...
        while i < len(train_loader):
            ...
            if i % params.displayInterval == 0:
                ...
                f.write(str(total_steps)+' '+str(params.niter)+' '+str(i)+' '+str(len(train_loader))+' '+str(loss_avg.val())+'\n')
                loss_avg.reset()
            if i % params.valInterval == 0:
                acc = val(crnn, test_dataset, criterion)
        if (total_steps + 1) % params.saveInterval == 0:
            torch.save(crnn.state_dict(), '{0}/crnn_rec_model_{1}_{2}.pth'.format(params.experiment, i, acc))
    f.close()
...
if __name__ == '__main__':
    ...
    test_dataset = dataset.lmdbDataset(
        root=args.valpath, transform=dataset.resizeNormalize((280, 32)))
