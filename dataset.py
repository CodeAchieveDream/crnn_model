...
import numpy as np
from imgaug_image import generate_image
import cv2
import traceback
...
if __name__ == '__main__':
    path = '/datassd/hzl/text_render_data/mingpian/lmdb_new/'
    data = lmdbDataset(path)
    for i in range(50):
        print(i)
        inx = random.randint(200, 4500000)
        img, label = data.__getitem__(inx)
        print(str(label, encoding='utf-8'))
        print(np.shape(img))
        img.save('./test_img/' + str(inx) + '_img.jpg', 'jpeg')
